...
/*   Updated: 2019/10/17 19:08:55 by lminta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
**	if (intensity < 0)
**		return ;
**	t->color =  ((int)(intensity * 255)) | ((int)(intensity * 255)) << 8 |
**((int)(intensity * 255)) << 16 | (0 << 24);
...
**		B.x = second_half ? t1.x + (t2.x - t1.x) * beta : t0.x + (t1.x - t0.x)
** beta;
**		B.y = second_half ? t1.y + (t2.y - t1.y) * beta : t0.y + (t1.y - t0.y)
** beta;
**		B.z = second_half ? t1.z + (t2.z - t1.z) * beta : t0.z + (t1.z - t0.z)
** beta;
**		if (A.x > B.x)
**			point_swap(&A, &B);
**		j = A.x;
...
**			p.y = A.y + (B.y - A.y) * phi;
**			p.z = A.z + (B.z - A.z) * phi;
**			if (((p.x > 0 && p.x < surface->width) && (p.y > 0 && p.y <
** surface->height)))
